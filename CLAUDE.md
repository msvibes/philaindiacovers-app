...
- **When handling any Supabase API key or credential, always decode it (JWT payload) and verify its actual `role` claim before use, writing to a file, or embedding in a build — checking presence, length, or the variable's own name is not sufficient verification.** This applies to every future session, not just when something feels risky. Learned from a real incident, 2026-09-03: during a service-role-key rotation, the new service-role key and the new anon key got crossed between `.env` and `.env.production` — a value named `VITE_SUPABASE_ANON_KEY` was actually the service-role key, and it was built into and delivered as part of a real installer before being caught. The only verification performed up to that point was confirming a value was present and checking its length — never decoding the token itself. `scripts/check-no-secret-leak.mjs` was fixed the same day to decode any JWT-shaped string and check its own `role` claim directly, rather than trusting a prefix or a variable name.
...
GitHub Actions (`.github/workflows/ci.yml`) runs on every PR to `main`: lint, typecheck, the full Vitest suite (unit + live integration), then a build (which also runs the secret-leak guardrail, `scripts/check-no-secret-leak.mjs`, scanning tracked files and `out/` for both the `sb_secret_...` key pattern and — since 2026-09-03, see the credential-verification gotcha above — any JWT-shaped string whose decoded payload has `role: "service_role"`). Mirrors the Admin repo's own CI gate (`docs/Test-Strategy.md`'s "CI Gate" section is shared across both repos).
