...
| T-29 | EPIC-11 | Build the Home/landing screen (FR-01/US-43) — welcome content, "Enter the catalogue" CTA, per the approved prototype direction | Added 2026-08-24 — **process gap identified**: FR-01 is `Required` in Trial Readiness and P0 in §6.1, but was never given a task ID in the original T-10.3 decomposition, despite T-10/T-11/T-12 (which would have naturally included it) being removed during the PRD-v1.0 reconciliation. Same root cause as T-28's gap: a requirement surviving a scope change without anyone re-checking it still has an owner | Home screen renders on launch, distinct from the catalogue grid, with a working entry point into it |
...
| T-31 | EPIC-18 | Build the native Electron menu bar (FR-26/US-54) via Electron's Menu API | Added 2026-08-24, same process-gap category — this one specifically fell through the "fold into whichever task touches that screen" logic because it isn't tied to any screen at all; it's main-process wiring, not renderer UI | Menu bar renders natively (not in-page HTML) with File/Edit/View/Help |
...
| T-22 | EPIC-17 | Build "browse by year" timeline view grouping covers by year | — | Selecting a year filters the grid to that year only |
...
| T-24 | EPIC-18 | Build native Electron menu bar (File/Edit/View/Help) via Electron's Menu API | Do not build this as in-page HTML — must be a real native menu | Menu bar renders natively on the app window, not inside the web content area |
