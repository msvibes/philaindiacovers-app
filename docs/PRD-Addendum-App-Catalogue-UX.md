...
- [ ] Collector can sign up, log in, and reach the catalogue (per US-01–05, implemented against the real backlog, not this addendum)
- [ ] Grid, filters, detail view, and navigation match the agreed design references and real field names (Postal Circle, Product Category, Date of Issue)
- [ ] Offline shows cached data with a banner, not a hard failure
- [ ] Update-available prompt fires correctly against a real new GitHub Release
...
|---|---|---|---|---|
| Individual collector | Windows desktop, friends/family trial | Browse and search India's GI Tag Special Covers catalogue | Current build is a bare, unstyled proof-of-pipeline | Mixed — trial group isn't necessarily technical |
...
*Still intentionally undecomposed: the interactive guided tour (needs the rest of the UI stable first, since it points at real elements), the update-available prompt (blocked on the GitHub Releases feed dependency in §5.4), and export/print (straightforward once the rest lands — low risk to leave for a later pass). Toasts, password-visibility toggle, terms checkbox, logout confirmation, shortcuts help, recently viewed, and the GI registry link (FR-22–FR-27, FR-33) are all small enough to fold into whichever task touches that screen, rather than each getting a dedicated task. **Correction (2026-08-23):** FR-11–14 were originally assumed to fall into this same "fold into whichever task touches the screen" category, but Claude Code correctly identified during T-13+T-18 planning that FR-12 specifically requires lifting state out of `Catalogue.tsx` into `App.tsx` — a real architectural change, not a same-screen tweak. Given its own task, T-25, sequenced after T-13+T-18.*
