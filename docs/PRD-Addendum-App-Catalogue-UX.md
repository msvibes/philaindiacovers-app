...
| T-33 | EPIC-11 | **Replace the account-menu dropdown with a persistent left sidebar.** Added 2026-08-25 after live use of the dropdown (T-29/T-24) — product owner correctly identified that hiding primary navigation behind a click-to-reveal icon is the wrong pattern for this small a destination set (3 real screens + 3 placeholders + logout); a persistent sidebar is the standard pattern for this class of desktop app. Full exact specification, no ambiguity left for implementation: sidebar is `width:220px` fixed (no collapse/responsive behavior — deliberate scope boundary, not a gap), `height:100%`, `background:var(--card)`, `border-right:1px solid var(--line)`, `display:flex;flex-direction:column`. Brand header at top: `padding:20px 20px 24px`, "PhilaIndiaCovers" in `var(--font-display)`/600/18px/`var(--ink)`, `border-bottom:1px solid var(--line)`. Primary nav rows (Home/Catalogue/Settings): `padding:10px 20px`, `var(--font-sans)`/13.5px; inactive `color:var(--ink-soft)`; active (matches `App.tsx`'s current `screen` state) gets `background:var(--paper)`, `color:var(--ink)`, `font-weight:500`, `border-left:3px solid var(--stamp)`; hover on inactive rows only, `color:var(--ink)`. Divider `1px solid var(--line)`, `margin:12px 20px`. Coming-soon rows (Collection Manager/Wish List/My Progress): same padding/font, `color:var(--ink-soft)`, `opacity:0.5`, "— Coming soon" suffix, `cursor:default`, zero interactivity — no hover, no click handler. Another divider. Log out: `margin-top:auto` (bottom-anchored regardless of content above), `border-top:1px solid var(--line)`, `padding:14px 20px`, `color:var(--ink)`. Content area: `margin-left:220px`, `flex:1`. The old full-width header row (brand + gear icon) is removed entirely from every screen — Catalogue's search/Filters/sort row survives but becomes a page-level toolbar inside the content area, not a persistent app-wide header. When `Detail` is open (overlay state per T-25/T-29), sidebar still highlights whichever `screen` was active underneath — never shows nothing active. **Two additions found 2026-08-25 during a full nook-and-corner audit, folded in since they touch this same sidebar:** (a) US-55 (keyboard shortcuts) was originally scoped assuming it lived in the dropdown this task removes — add a "Keyboard Shortcuts" row to the sidebar's coming-soon-adjacent section (between the primary nav divider and the coming-soon divider, styled like a primary nav row since it's real/functional, not a placeholder) that opens the existing shortcuts view; (b) FR-25 (logout confirmation) was decided but never built by any task — clicking "Log out" must show a confirm step ("Log out?" / Cancel / Log out) before actually ending the session, not log out immediately on click | This spec is exhaustive on purpose, learning directly from T-24's "matches the pattern" miss that shipped an inconsistent trigger button. Do not build this from a paraphrased description — implement the literal values above. No responsive/collapse behavior — do not add one speculatively. Coming-soon items must have zero interactivity, not just visual greying | Sidebar renders identically and persistently on Home, Catalogue, Detail, and Settings; active-state highlighting correctly tracks the current screen at all times, including with Detail open; coming-soon items are confirmed non-interactive (no hover, no click); a side-by-side screenshot comparison against this spec's values is provided before calling this done, not just a description of the result. **Consistency audit, added 2026-08-25, folded into this task rather than done separately since T-33 already touches every screen's DOM structure:** confirm no element anywhere renders in a browser-default font (every heading is Playfair Display, every UI/body element Inter, every ID/data value IBM Plex Mono); confirm `var(--stamp)` appears only at meaningful moments (active nav indicator, Verified checkmark) and not as general decoration, and no hardcoded black/grey exists where `var(--ink)`/`var(--ink-soft)` should be used; confirm left-alignment is the default for card text/list items/form labels, with centering reserved only for deliberate standalone moments (empty states, Home's hero block); confirm consistent spacing rhythm across screens (the established scale uses values like `20px`/`14px`/`12px`) with no arbitrary one-off values. Report specific findings for each category, not a blanket "checked, looks fine". **Two findings from the 2026-08-26 Lovable UI comparison, folded in:** (c) add a small uppercase eyebrow label above each screen's main heading, in `var(--stamp)`, ~11px, letter-spacing ~0.05em — "THE ARCHIVE" above Catalogue's heading, "COLLECTOR'S DESK" above Home's "Welcome back!", "SPECIAL COVER" above Detail's Cover Name, "PREFERENCES" above Settings — a cheap, genuinely good addition, confirmed working well in the comparison prototype; (d) when the Keyboard Shortcuts modal is open, its own sidebar row should show the active-highlight state (not Catalogue/whichever screen is underneath) — and revert to the real current screen's highlight the moment the modal closes, mutually exclusive, never both at once |
| T-34 | EPIC-11 | FR-04/US-06 — the interactive guided tour. Added 2026-08-25: this has sat "undecomposed, needs the rest of the UI stable first" since the very first task breakdown — the UI is stable now (Home, Catalogue, nav all exist), so this is buildable. Highlights real UI elements on first login (interactive walkthrough, not static slides, per the original product owner decision) — Home's CTA, Catalogue's search/filters, a real card, the sidebar (once T-33 lands) | Sequence after T-33, not before — no point building a tour that highlights the dropdown-based nav this session just decided to replace | Appears once per new account on first login, skippable, does not reappear after completion, points at real live elements matching whatever T-33 ships |
| T-35 | EPIC-18 | FR-24/US-56 — toast/confirmation feedback. Added 2026-08-25: decided early this session, never actually built by any task despite several touching relevant screens (filters, exports once they exist, favorites-adjacent actions). Cross-cutting UI pattern (a small `Toast.tsx` + a shared trigger function), not tied to one screen | Do not scope this narrowly to one screen's actions — build the shared mechanism once, usable from anywhere | A toast appears and auto-dismisses for at least: filter applied (Catalogue), and is genuinely reusable (not copy-pasted) for future actions like export completion |
| T-36 | EPIC-11 | Style the splash/session-loading state — `App.tsx`'s `session: 'loading'` value currently renders unstyled (likely a blank flash) while the real Supabase session check resolves. Added 2026-08-25, confirmed in scope. Exact spec, reusing the pattern already established in the v2/v3 prototypes: `min-height:70vh`, `display:flex;flex-direction:column;align-items:center;justify-content:center;text-align:center`. Heading "PhilaIndiaCovers" — `var(--font-display)`/600/22px. Subtext "Connecting..." — 13px/`var(--ink-soft)`. Three bouncing dots below: each `6px` circle, `background:var(--stamp)`, staggered bounce animation (`animation-delay` 0/0.15s/0.3s on a 1.1s ease-in-out infinite bounce) | This must reflect the real, variable-length session check — not a fixed fake delay like the prototype's ~1.4s timeout. If the real check is near-instant, this state may barely be visible, and that's correct, not a bug to "fix" by adding an artificial minimum display time | Splash renders with the exact values above during real session resolution; no unstyled/blank flash between app launch and Login or Home appearing |
| T-37 | EPIC-18 | Generic error boundary — added 2026-08-25, confirmed in scope. No fallback exists today for an unexpected render crash (a genuine bug, not a handled offline/error state) — currently means a blank or broken screen with no recovery path. A React Error Boundary wrapping `App.tsx`'s top level, catching errors from any screen (Login/Signup/Home/Catalogue/Detail/Settings alike) | This is a safety net for genuine bugs, not a replacement for proper error handling elsewhere in the app (offline banners, form validation, etc. all stay as their own real handling) — do not use this as an excuse to skip specific error states elsewhere | Fallback UI matches the app's actual design tokens (not a generic browser error page) — a calm message plus a "Reload" action that restarts the app cleanly. Verified by deliberately triggering a real render error and confirming the fallback appears instead of a blank screen |
| T-38 | EPIC-18 | Windows installer packaging (electron-builder) — added 2026-08-27. Currently unmodified electron-vite scaffold: generic `appId: com.electron.app`, mac/linux targets configured despite this being Windows-only, no CI exercise of the actual `electron-builder` package step, fake `publish` URL. Fix `electron-builder.yml`: Windows/nsis target only, a real appId (e.g. `com.krutimlogic.philaindiacovers`), a real branded icon (replace the default Electron atom logo — use `docs/legal/DISCLAIMER-and-Developer-Details.md`'s already-confirmed publisher identity for branding direction), remove the fake publish URL or point it at the real GitHub Releases feed per §5/distribution decision. **Two decisions resolved, not to be re-litigated:** (1) ship unsigned for Trial #1 — no code-signing certificate, same cost-deferral reasoning as Apple SSO; testers will see Windows SmartScreen's "Unknown Publisher" warning, dismissible via "More info → Run anyway" — document this for testers, don't try to route around it. (2) `publisherName: "Krutim Logic"` and the EULA content are already fully decided in `docs/legal/DISCLAIMER-and-Developer-Details.md` (confirmed weeks earlier in this project) — use them directly, don't placeholder or wait for T-27. Create `LEGAL/EULA.txt` from that already-drafted content and wire it to `nsis.license` as part of this task. **Icon resolved 2026-08-27, explicitly a placeholder:** product owner provided a real image file (200×200 JPG, "INDPHILA" wordmark, red/blue reasonably close to the established `--stamp`/`--ink` palette) — use it to generate the required `.ico`/`.icns`/`.png` sizes. This is a known, deliberate placeholder (different name from the app itself), not final branding — confirm 200×200 upscales acceptably for a proper multi-resolution `.ico` (commonly needs up to 256×256); flag directly if it doesn't rather than silently using a degraded result | **Scope boundary vs. T-27, resolved 2026-08-27:** T-38 owns packaging mechanics and already-decided static values (appId, icon, publisherName, EULA file content). T-27 still owns the in-app disclaimer screen/component, Settings integration, and the About/developer-info display — real UI work, not packaging. Do not build T-27's UI here; do not leave publisherName/EULA as placeholders waiting for T-27 either, since both are already decided | A real `.exe` installer builds successfully via `electron-builder`, actually installs on a real Windows machine, shows the correct app name/icon (not the default Electron atom), and the NSIS license/EULA page displays the real content before allowing install to proceed. CI exercises the actual packaging step, not just `electron-vite build` |
| T-39 | EPIC-18 | **Separate dev/CI Supabase project from production (KAN-69).** Added 2026-08-30 — this Jira issue existed since 2026-08-24 but was never written into this addendum, making it invisible to Claude Code (no Jira access). Real risk, unchanged since creation: a single Supabase project (`hcaivtygzwjemjngcmji`) serves both the real app and every CI run's live integration tests (`createUser`/`deleteUser` calls). Every PR merged since T-13+T-18 (T-14, T-25, T-28, T-29, the CSS fix, T-33, T-38 — 8+ merges) has run CI against this same shared project. If a test ever fails before its `afterAll` cleanup completes, orphaned test accounts land in the same data real trial users will use. Once a genuinely separate dev project exists, all three `ci-dev-supabase` GitHub Actions secrets need manual re-verification that they point at the new dev project, not production — nothing in the CI workflow itself would catch a misconfiguration; it would silently start running live tests against production | This is real infrastructure work (a new Supabase project, migrating schema/RLS policies, re-pointing CI secrets), not a config toggle. Per the 2026-08-25 decision already logged on KAN-69 itself: Claude Code leads execution, product owner verifies each major step rather than executing manually, given the failure mode (CI silently pointing at production) is higher-stakes than typical verification this project has done. Investigate and plan first — do not touch production or existing CI secrets speculatively | A genuinely separate Supabase project exists for `ci-dev-supabase`; all three CI secrets confirmed pointing at it, not production; a real CI run confirmed still passing against the new project; the original shared-project risk is closed, not just relocated |
| T-40 | EPIC-18 | **Configure a real custom SMTP provider (KAN-70).** Added 2026-08-30, same "existed in Jira, invisible to Claude Code" gap as T-39. **Current real state, not the Aug-24 original**: Supabase's default 2-emails/hour relay was hit almost immediately once real testing started. Resend was configured as a first attempt but hit its own sandbox restriction (can only deliver to the account's own verified address until a sending domain is verified) — domain verification via `mail.philapedia.club` was started but never completed (DNS records were never actually added). **Gmail SMTP (`krutimlogic@gmail.com`, an App Password) is what's actually configured and working right now** — this unblocked real testing but was always logged as a deliberate stopgap, not a real fix (personal account, not built for production/trial signup volume, real risk of Google flagging automated sending patterns). Real fix still needed: a genuine transactional provider (Resend, SendGrid, Postmark) with a verified sending domain — either finish the abandoned `mail.philapedia.club` Resend verification, or start fresh with a different provider/domain. **Scope addition, found 2026-08-30 during T-39's CI verification**: the *new* dev/CI Supabase project (`gsdnjzfgtvasxsslemwn`, created by T-39) inherited the same default 2/hour rate limit and already caused one real CI failure (`signup.integration.test.ts`, App PR #17/CI) — this isn't just a production-signup problem anymore. T-40 must configure real SMTP (or at minimum a raised rate limit) on the **dev/CI project too**, not only wherever production email ends up, or this exact flakiness will keep recurring unpredictably on future PRs with heavy live-integration-test activity | Do not simply raise Gmail's implicit limits further or treat the current working state as sufficient — it was always explicitly logged as a stopgap. If resuming the Resend path, the DNS records were never added to `philapedia.club`'s DNS (via Hostinger) — that step, not the Resend account itself, is where it was left off. Do not fix only the production-bound project and consider this done — the dev/CI project needs its own resolution too | A real transactional SMTP provider is configured with a verified sending domain (not the sandbox-restricted default sender), on **both** the production-bound and dev/CI projects; a real signup test confirms delivery to an arbitrary email address, not just the account owner's own address; Supabase's rate limit is raised to a real, provider-appropriate value on both projects, not the emergency-unblock value set on 2026-08-24 |
...
| T-21 | EPIC-17 | Build "browse by region" view using a verified India states TopoJSON/GeoJSON dataset and a real mapping library (e.g. react-simple-maps) — do not hand-code state boundary SVG paths. **Sourcing fully resolved (2026-08-26) — see notes below.** Recommended source: **geoBoundaries India ADM1** (`admUnitCount:36`, confirmed directly via `https://www.geoboundaries.org/api/current/gbOpen/IND/ADM1/`), sourced from DataMeet India community + Election Commission of India, updated 2023, licensed **CC BY 2.5 India** (confirmed via the same API — genuinely open, no commercial-use restriction). Real download links from the API: `gjDownloadURL` and `tjDownloadURL` point to `github.com/wmgeolab/geoBoundaries/raw/9469f09/releaseData/gbOpen/IND/ADM1/...` — **these are Git-LFS-tracked** (raw `curl`/`wget` against `raw.githubusercontent.com` returns only an LFS pointer, not real content — confirmed directly, do not repeat this dead end). Download via a real LFS-aware method instead: `git lfs clone`, a real browser download from GitHub's UI (resolves LFS correctly), or the API's `staticDownloadLink` (a packaged zip release asset, may bypass the LFS blob issue). Once downloaded, simplify via mapshaper targeting ~100–300 KB TopoJSON, same as originally planned. **Content cross-validated, not just source-trusted:** direct content inspection could not be completed on this specific file (LFS-blocked), but an independently-sourced dataset from a completely different lineage (Highcharts' map collection, OpenStreetMap-derived) was fetched and read in full — confirmed 36 regions, Ladakh separate from Jammu and Kashmir, Odisha/Uttarakhand (current names), Telangana present, and the correct 2020 Dadra-Nagar-Haveli/Daman-Diu merger. Two independent sources agreeing is real corroborating evidence for the 36-region current boundary set generally — **still verify the actual geoBoundaries file's own properties directly once downloaded**, don't assume the cross-validation substitutes for checking the real file you're actually shipping.<br><br>**Explicitly ruled out — do not use, do not re-investigate:** (1) `udit-001/india-maps-data` — district-level, wrong granularity. (2) `geohacker/india`'s `state/india_state.geojson` — 21.9MB unsimplified AND outdated (undivided J&K, "Orissa," "Uttaranchal," missing Telangana). (3) **Highcharts' map collection** (`code.highcharts.com/mapdata/countries/in/in-all.topo.json`) — content is genuinely excellent and was fully, directly verified (all 4 accuracy checks pass), **but licensing is disqualifying**: their own `LICENSE.md` states "the use of the maps in this collection requires a valid Highcharts Maps license" — not the CC BY 3.0 open license initially assumed; do not use this file in the shipped app regardless of its accuracy. (4) India Geodata's `Admin2.shp` (the previously-recommended lead) — superseded by geoBoundaries directly serving the same underlying DataMeet data with a clearer license and a documented API, no need to go through the shapefile-conversion path originally planned | State boundary accuracy is non-negotiable given the sensitivity of India's political map; do not add district-level or higher-precision data than the state-level view needs; do not substitute the Highcharts file for convenience despite its verified accuracy — the license blocker is real | Every state renders correctly and matches an authoritative reference map, including the current J&K/Ladakh split (verified directly in the actual downloaded GeoJSON/TopoJSON's own properties — not assumed from the API metadata or the cross-validation reasoning above); clicking a state filters the grid to that region; the bundled asset is confirmed present in the built installer and the view works fully offline |
