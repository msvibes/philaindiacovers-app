...
## ADR-009: Separate Dev/CI and Production Supabase Projects, Selected by Vite's Own Mode-Based Env Precedence

**Status:** Accepted

**Context:** For the first several weeks of this project, one Supabase project (`hcaivtygzwjemjngcmji`) served double duty as both the real app's backend and the target for every CI run's live integration tests (`createUser`/`deleteUser` cycles, RLS checks, etc.) in both this repo and `philaindiacovers-admin` — a real, unchanged-since-2026-08-24 risk (8+ merges ran CI against it before it was fixed): if a test ever failed before its `afterAll` cleanup ran, orphaned test accounts and data would land in the same project real trial users depend on. This matched the original documented design intent (`Admin/docs/AI-Agent-Implementation-Brief.md` §10.4, "Environment separation") to eventually split dev/test traffic from production — just not yet executed.

**Decision:** T-39/KAN-69 (2026-08-31) created a genuinely separate Supabase project for dev/CI use (`gsdnjzfgtvasxsslemwn` — a new Free-tier project; Supabase's free-tier 2-active-projects-per-org allowance was verified live against their current pricing page before relying on it, not assumed). The original project (`hcaivtygzwjemjngcmji`) became production-only. All 20 migrations were replayed against the new project and parity-verified (row counts, RLS policy counts) before any CI secret was repointed; both repos' full local test suites were run for real against the new project's live credentials as the final check. `philaindiacovers-admin` was brought into the same split in the same session — an identical shared-project risk existed there too, and fixing only this repo would have left Admin's CI still writing against what was about to become production.

Credential **selection** at build time (added 2026-09-03, after KAN-72's investigation — see Consequences) deliberately reuses electron-vite's standard pass-through of Vite's own mode-based env-file precedence, rather than any custom logic: `.env` (dev/CI project's credentials) is loaded for both `dev`/`serve` (`mode: 'development'`, which also picks up `.env.development` if present) and as the fallback base for `build` (`mode: 'production'`); a separate, never-committed `.env.production` holds the real production credentials and is loaded only for `build`, with its values overriding `.env`'s matching keys. Neither env file is committed — `.env.example`/`.env.production.example` document the required keys.

**Consequences:** CI and local dev traffic — including throwaway test-account creation/deletion — now never touches production data; the split is real and independently verified (own migrations, own RLS/grants parity, own `ci-dev-supabase` GitHub Environment secrets per repo), not just relocated on paper. A schema change must be applied to both projects explicitly — nothing propagates automatically between them.

**A real cost, discovered empirically rather than anticipated:** because `.env.production` is optional from Vite's own perspective (a missing file just means fewer overrides, not an error), a `build`/`build:win` run with no `.env.production` present silently falls back to `.env` — the dev project's credentials — rather than failing loudly. This exact gap meant **every installer built between T-39 (2026-08-31) and this fix (2026-09-03) silently pointed at the dev project instead of production**, confirmed by building with a placeholder-credentialed `.env.production` and watching a real installed build fail to sign in against a deliberately non-resolving host (full verification trail: KAN-72's closing comment, `.claude/PROGRESS.md`'s 2026-09-03 entry). **Standing practical implication, restated here as this decision's real cost, not just a `CLAUDE.md` tip:** always confirm `.env.production` holds real production credentials before running a release build — there is no automated guard against silently shipping dev credentials in a production installer.

**A known, accepted residual risk, not closed by this decision:** this repo and `philaindiacovers-admin` currently share the *same* dev/CI project, each with its own independent per-repo CI concurrency group — a PR pushed to both repos at once could run live integration tests against that shared project concurrently. Accepted for now given both repos' genuinely small live-data footprint. Trigger condition for revisiting: if Supabase Auth rate-limit errors are ever actually observed in either repo's CI logs, build real cross-repo coordination then, not before.
